...
class HexView(View):
    ...
    def html_rowheader(self, address):
        return "<span class=\"row-header\">0x{0:08x}: </span>".format(address)
    def html_bytedata(self, address, byte, specialclass=""):
        return "<span id=\"{0:08x}-data\" class=\"byte-data {1:s}\">{2:02x}</span>".format(address, specialclass, byte)
    def html_byte_ascii(self, address, byte, specialclass=""):
        return "<span id=\"{0:08x}-ascii\" class=\"byte-ascii {1:s}\">{2:s}</span>".format(address, specialclass, self.get_visible_ascii(chr(byte)))
    ...
    def get_visible_ascii(self, char):
        #to do: 1. creating this list each time is no efficient.
        #       2. also, shoulb be moved to helper-functions
